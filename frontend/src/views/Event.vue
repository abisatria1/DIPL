<template>
    <div>
        <b-card title="Event">
            <b-button href="#" variant="primary" @click="createEvent" class="mb-3">Add Event</b-button>
            <b-table striped hover :items="items" :fields="fields">
              <template #cell(template_twibbon)="dataEvent">
                <img :src="baseUrl + '/'+ dataEvent.item.template_twibbon" alt="template_twibbon" width="100" height="100">
              </template>
              <template #cell(id_event)="dataEvent">
                <b-button @click="editEvent(dataEvent.item.id_event,$event.target)" variant="warning" >Edit Event</b-button>
                <b-button @click="deleteEvent(dataEvent.item.id_event,$event.target)" class="ml-3" variant="danger" >Delete Event</b-button>
              </template>
            </b-table>
...
import axios from "axios" 
  ...
  export default {
    ...
    data() {
      return {
        fields:['nama_event','date_event','jumlah_participant', "deskripsi_event" ,"template_twibbon", {key:'id_event',label:"Actions"}],
        items: []
      }
    },
    methods: {
